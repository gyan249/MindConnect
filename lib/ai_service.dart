...
Future<String> fetchAIResponse(
  ...
  try {
    ...
    String historyText = chatHistory
        .map((m) => "${m['isMe'] ? 'User' : 'AI'}: ${m['text']}")
        .join('\n');

    // Prevent Gemini rate limit (429 error)
        ...
        "contents": [
          {
            "parts": [
              {
                "text":
                    "You are a helpful mental wellness companion. Keep your answers short and supportive.\n\n$historyText\nUser: $userMessage"
              }
            ]
          }
        ]
...
/// ===============================
Future<Map<String, dynamic>> fetchWellnessRecommendations(
    String ageGroup, int score, int maxScore) async {
